...
proc sql;
select count(*) as Total_AEs, sum(AESER = 'Yes') as Total_serious_AEs, sum(SEVERITY = 'Severe') as Total_severe_AEs
from adevent;
quit;
...
/*Create a Top 5 Most Common AEs table using SQL:
| Rank | AEDECOD | Count |*/

title "Top 5 Most Frequent Adverse Events";
proc sql outobs=5;
